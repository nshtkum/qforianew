...
def call_perplexity(query):
    ...
    try:
        response = requests.post("https://api.perplexity.ai/chat/completions", headers=headers, json=data, timeout=15)
        response.raise_for_status()
...
def research_missing_context(missing_items):
    results = []
    for item in missing_items[:5]:  # Limit to 5 for simplicity
